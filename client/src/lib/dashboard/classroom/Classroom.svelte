<script>
  ...
  const checkIfUserInTeamInThisClassroom = async (/** @type {any} */ g_id) => {
    try {
...
<main class="container">
  <header>
    ...
    <hgroup class="right">
      ...
      <h4>
        Created On {convertDateToHumanReadable(activeClassroom.created_at)}
      </h4>
    </hgroup>
  </header>
  <section>
    ...
    {:then data}
      ...
      {:else}
        <div>
          ...
          <p>
            You are not part of any team in this classroom. Either create a team
            or wait for a request from another team.
          </p>
        </div>
        <div class="grid">
    ...
    {:catch error}
      <div>
        <h3>Error</h3>
        <p>There was an error fetching the team data: {error.message}</p>
      </div>
    {/await}
  </section>
</main>
